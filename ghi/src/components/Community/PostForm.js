...
function PostForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        const postURL = 'http://localhost:8001/post/'
        const fetchConfig = {
            method: "post",
                ...
                <div className="shadow p-4 mt-4">
                  <div className="card-body">
                    ...
                    <div className="form-floating mb-3">
                        <input onChange={titlechange}  required placeholder="Title" type="text" id="title" name="title" className="form-control" value={title}/>
                    ...
                    <select onChange={gamechange} required id="game" name="game" className="form-select" value={game}>
                            <option className="dropdown-3" value="">Choose a game</option>
                            {games.map(gamess => {
                                return (
                            <option className="dropdown-3" key={gamess.id} value={gamess.id}>
                            {gamess.title}
                            </option>
                    );
                    })}
